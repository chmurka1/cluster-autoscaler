...
func TestScaleSetMaxSize(t *testing.T) {
	...
	assert.Equal(t, provider.NodeGroups(context.TODO())[0].MaxSize(context.TODO()), 5)
}
...
func TestScaleSetMinSize(t *testing.T) {
	...
	assert.True(t, registered)
	assert.Equal(t, len(provider.NodeGroups(context.TODO())), 1)
	assert.Equal(t, provider.NodeGroups(context.TODO())[0].MinSize(context.TODO()), 1)
}
...
func TestScaleSetMinSizeZero(t *testing.T) {
	...
	assert.True(t, registered)
	assert.Equal(t, len(provider.NodeGroups(context.TODO())), 1)
	assert.Equal(t, provider.NodeGroups(context.TODO())[0].MinSize(context.TODO()), 0)
}
...
func TestScaleSetTargetSize(t *testing.T) {
	...
	for _, orchMode := range orchestrationModes {
		...
		targetSize, err := provider.NodeGroups(context.TODO())[0].TargetSize(context.TODO())
		assert.NoError(t, err)
		...
		targetSize, err = provider.NodeGroups(context.TODO())[1].TargetSize(context.TODO())
		assert.NoError(t, err)
...
func TestScaleSetIncreaseSize(t *testing.T) {
	...
	for _, orchMode := range orchestrationModes {
		...
		// Increase 2 nodes.
		err = provider.NodeGroups(context.TODO())[0].IncreaseSize(context.TODO(), 2)
		assert.NoError(t, err)
		...
		targetSizeForEdgeZone, err := provider.NodeGroups(context.TODO())[1].TargetSize(context.TODO())
		assert.NoError(t, err)
		...
		err = provider.NodeGroups(context.TODO())[1].IncreaseSize(context.TODO(), 2)
		assert.NoError(t, err)
		...
		assert.Equal(t, len(provider.NodeGroups(context.TODO())), 3)

		// Current target size is 0.
		targetSizeForEdgeZoneMinZero, err := provider.NodeGroups(context.TODO())[2].TargetSize(context.TODO())
		assert.NoError(t, err)
		assert.Equal(t, 0, targetSizeForEdgeZoneMinZero)

		mockVMSSClient.EXPECT().CreateOrUpdate(gomock.Any(), provider.azureManager.config.ResourceGroup,
			"edgezone-minzero-vmss", gomock.Any()).Return(nil, nil).AnyTimes()
		err = provider.NodeGroups(context.TODO())[2].IncreaseSize(context.TODO(), 2)
		assert.NoError(t, err)
		...
		go func() {
			...
			for {
				...
				case <-stop:
					return
				default:
					_, _ = ng.TargetSize(context.TODO())
				}
			}
		}()
	}
...
func TestScaleSetAtomicIncreaseSize(t *testing.T) {
	...
	ss := newTestScaleSet(provider.azureManager, "test-asg-doesnt-exist")
	err = ss.AtomicIncreaseSize(context.TODO(), 1)
	expectedErr := fmt.Errorf("could not find vmss: test-asg-doesnt-exist")
	assert.Equal(t, expectedErr, err)
	...
	err = provider.NodeGroups(context.TODO())[0].AtomicIncreaseSize(context.TODO(), -1)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "size increase must be positive")

	// Error: zero delta
	err = provider.NodeGroups(context.TODO())[0].AtomicIncreaseSize(context.TODO(), 0)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "size increase must be positive")

	// Error: exceeds max size (max is 5, current is 3, delta 3 = 6 > 5)
	err = provider.NodeGroups(context.TODO())[0].AtomicIncreaseSize(context.TODO(), 3)
	assert.Error(t, err)
	...
	err = provider.NodeGroups(context.TODO())[0].AtomicIncreaseSize(context.TODO(), 2)
	assert.NoError(t, err)
...
func TestScaleSetIncreaseSizeOnVMProvisioningFailed(t *testing.T) {
	...
	for testName, testCase := range testCases {
		t.Run(testName, func(t *testing.T) {
			...
			scaleSet, ok := provider.NodeGroups(context.TODO())[0].(*ScaleSet)
			assert.True(t, ok)

			// Increase size by one, but the new node fails provisioning
			err = scaleSet.IncreaseSize(context.TODO(), 1)
			assert.NoError(t, err)

			nodes, err := scaleSet.Nodes(context.TODO())
			assert.NoError(t, err)
...
func TestScaleSetDeleteNodes(t *testing.T) {
	...
	for _, tc := range cases {
		...
		targetSize, err := scaleSet.TargetSize(context.TODO())
		assert.NoError(t, err)
		assert.Equal(t, 3, targetSize)
		...
		err = scaleSet.DeleteNodes(context.TODO(), nodesToDelete)
		assert.NoError(t, err)
...
func TestScaleSetForceDeleteNodesDoesNotPublishNegativeCachedSize(t *testing.T) {
	...
	err = scaleSet.ForceDeleteNodes(context.TODO(), nodesToDelete)
	assert.NoError(t, err)
...
func TestScaleSetDeleteNoConflictRequest(t *testing.T) {
	...
	err = scaleSet.DeleteNodes(context.TODO(), []*apiv1.Node{node})
}
...
func TestScaleSetId(t *testing.T) {
	...
	assert.True(t, registered)
	assert.Equal(t, len(provider.NodeGroups(context.TODO())), 1)
	assert.Equal(t, provider.NodeGroups(context.TODO())[0].Id(), "test-asg")
}
...
func TestAgentPoolDebug(t *testing.T) {
	...
	assert.Equal(t, asg.Debug(context.TODO()), "test-scale-set (5:55)")
}
...
func TestScaleSetNodes(t *testing.T) {
	...
	for _, orchMode := range orchestrationModes {
		...
		assert.True(t, ok)
		assert.NotNil(t, ss)
		instances, err := group.Nodes(context.TODO())
		assert.NoError(t, err)
...
func TestScaleSetTemplateNodeInfo(t *testing.T) {
	...
	t.Run("Checking fallback to static because dynamic list is empty", func(t *testing.T) {
		...
		nodeInfo, err := asg.TemplateNodeInfo(context.TODO())
		assert.NoError(t, err)
...
func TestScaleSetCseErrors(t *testing.T) {
	...
	manager.RegisterNodeGroup(
		newTestScaleSet(manager, "test-asg"))
	manager.explicitlyConfigured["test-asg"] = true
	scaleSet, _ := provider.NodeGroups(context.TODO())[0].(*ScaleSet)
...
func TestScaleSetIncreaseSizeWithETag(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			provider, err := BuildAzureCloudProvider(manager, nil)
			assert.NoError(t, err)
			scaleSet := provider.NodeGroups(context.TODO())[0].(*ScaleSet)
...
func TestScaleSetETagPreconditionFailureInvalidatesSizeCache(t *testing.T) {
	...
	target, err := scaleSet.TargetSize(context.TODO())
	assert.NoError(t, err)
...
func TestAtomicIncreaseSizeWithETag(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			err = scaleSet.AtomicIncreaseSize(context.TODO(), 1)
			assert.NoError(t, err)
